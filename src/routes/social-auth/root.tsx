...
const { VITE_GOOGLE_CLIENT_ID, VITE_GOOGLE_REDIRECT_URL_ENDPOINT } = import.meta.env;

const Root: React.FC = () => {
  const [username, setUsername] = useState<string | null>(localStorage.getItem('googleFirstName'));
  ...
  const openGoogleLoginPage = useCallback(() => {
    ...
    const params = new URLSearchParams({
      ...
      redirect_uri: `${VITE_GOOGLE_REDIRECT_URL_ENDPOINT}/home`,
    ...
    console.log('Google OAuth URL:', url); // Log the URL for debugging
  ...
  return (
    <div className="flex flex-col items-center justify-center h-screen">
      ...
      {username ? (
        ...
        <small className="text-primary-600">you not Logged in yet :eyes</small>
      )}
    </div>
  );
};
